...
def intelligence_scoring(pii: list, headers: list, ip_results: list,
    ...
    """
    Central intelligence scoring engine.
    Converts raw evidence into probabilistic attribution assessments.
    Output mirrors what an NIA analyst would write in an assessment report.
    """

    # ── Evidence weights ───────────────────────────────────────────────────
    evidence_scores = []

    def evidence(name, weight, found, detail=""):
        evidence_scores.append({
            "name":   name,
            "weight": weight,
            "found":  found,
            "detail": detail,
            "contribution": weight if found else 0,
        })
