...
export default function App(){
  ...
  const STEPS = [
    ...
    "Active IP Discovery — DNS resolution + Shodan queries...",
  ...
  const runAnalysis = useCallback(async () => {
    ...
    try {
      ...
      const discCount = result.active_ip_discovery?.discovered_ips?.length || 0;
      const headerLeaks = result.ip_intelligence?.filter(x=>isHeaderLeak(x.source||"")).length || 0;
      addLog(`DONE — PII:${result.pii?.length||0} | IPs:${ipCount} (${headerLeaks} header leaks, ${discCount} DNS-discovered) | Leads:${result.correlation?.high_confidence_leads?.length||0} | Graph:${g?.total_nodes||0}N/${g?.total_edges||0}E`, "ok");
  ...
  const TABS = [
    {id:"scan",          label:"① SCAN"},
    {id:"intelligence",  label:"★ INTELLIGENCE",          disabled:!report},
    {id:"results",       label:"② RESULTS",               disabled:!report&&!loading},
    {id:"opsec",         label:"⚠ OPSEC ANALYSIS",        disabled:!report},
    {id:"behavioral",    label:"◷ BEHAVIORAL",            disabled:!report},
    {id:"ip",            label:"⊕ IP INTELLIGENCE",       disabled:!report},
    {id:"discover",      label:"◎ ACTIVE IP DISCOVERY",   disabled:!report},
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"Courier New,monospace",fontSize:13}}>
      <style>{`@keyframes spin{from{transform:rotate(0)}to{transform:rotate(360deg)}}@keyframes blink{0%,100%{opacity:1}50%{opacity:0.2}}@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}*{box-sizing:border-box}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#1a3050}input,textarea,button{font-family:inherit}input::placeholder{color:#2a4a6b}a{text-decoration:none}`}</style>
      ...
      <div style={{padding:"10px 20px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",background:"rgba(0,0,0,0.8)"}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          ...
          <div>
            <div style={{color:C.cyan,fontSize:15,letterSpacing:5,fontWeight:"bold"}}>UMBRA  V3.2</div>
      ...
      <div style={{padding:20,maxWidth:1500,margin:"0 auto"}}>
            ...
            <div style={{gridColumn:"1/-1"}}>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
                {[
                  ...
                  ["ACTIVE IP DISCOVERY",C.green,"DNS resolution of ALL clearnet domains in page (href, src, CSP, JS). HackerTarget secondary resolver. Reverse IP. Shodan/Censys query generation."],
        ...
        {tab==="intelligence"&&(()=>{
          ...
          if (!intel) return <div style={{padding:"40px 0",textAlign:"center",color:"#4a7a9b"}}>Run a scan to see intelligence assessment.</div>;

          const confColor = intel.attribution_confidence_pct>=75?C.green:
                            intel.attribution_confidence_pct>=50?C.yellow:
                            intel.attribution_confidence_pct>=25?C.orange:C.red;
          const threatColor = {CRITICAL:C.red,HIGH:C.orange,MEDIUM:C.yellow,LOW:C.green}[intel.threat_level]||C.text;
          ...
          return (
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12,marginBottom:16}}>
                {[
                  ["ATTRIBUTION CONFIDENCE", `${intel.attribution_confidence_pct}%`, confColor],
                  ["ATTRIBUTION STRENGTH",   intel.attribution_strength,             confColor],
                  ["THREAT LEVEL",           intel.threat_level,                     threatColor],
                  ["OPERATOR OPSEC",         `${opsec?.opsec_score||0}/100`,         opsec?.opsec_score>=60?C.orange:C.red],
                  ["SOPHISTICATION",         intel.operator_sophistication,          C.purple],
                ].map(([label,val,color])=>(
                  <div key={label} style={{padding:"16px 14px",border:`1px solid ${color}35`,background:`${color}07`,textAlign:"center"}}>
                    <div style={{color:"#4a7a9b",fontSize:9,letterSpacing:1,marginBottom:8}}>{label}</div>
                    <div style={{color,fontSize:18,fontWeight:"bold",lineHeight:1.1}}>{val}</div>
                  </div>
                ))}
              </div>

              {/* Confidence bar */}
              <div style={{marginBottom:14,padding:"12px 16px",border:`1px solid ${confColor}30`,background:`${confColor}05`}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                  <span style={{color:confColor,fontSize:12,fontWeight:"bold"}}>Attribution Confidence: {intel.attribution_confidence_pct}% — {intel.attribution_strength}</span>
                  <span style={{color:C.text,fontSize:10}}>{intel.evidence_found_count}/{intel.evidence_total} evidence points</span>
                </div>
                <div style={{height:8,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden"}}>
                  <div style={{height:"100%",width:`${intel.attribution_confidence_pct}%`,background:confColor,transition:"width 1s ease",borderRadius:4}}/>
                </div>
                <div style={{color:"#4a7a9b",fontSize:10,marginTop:6}}>{intel.attribution_strength_note}</div>
              </div>

              {/* OPSEC score bar */}
              {opsec && (
                <div style={{marginBottom:14,padding:"12px 16px",border:`1px solid ${opsec.opsec_score>=60?C.orange:C.red}30`,background:`${opsec.opsec_score>=60?C.orange:C.red}05`}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    <span style={{color:opsec.opsec_score>=60?C.orange:C.red,fontSize:12,fontWeight:"bold"}}>Operator OPSEC: {opsec.opsec_score}/100 — {opsec.opsec_rating}</span>
                    <span style={{color:C.text,fontSize:10}}>{opsec.failure_count} failure(s) detected</span>
                  </div>
                  <div style={{height:8,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${opsec.opsec_score}%`,background:opsec.opsec_score>=60?C.orange:C.red,borderRadius:4}}/>
                  </div>
                  <div style={{display:"flex",gap:12,marginTop:6}}>
                    {Object.entries(opsec.by_severity||{}).map(([sev,cnt])=>cnt>0&&(
                      <span key={sev} style={{color:RISK_C[sev]||C.text,fontSize:10}}>{sev}: {cnt}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                {/* Operator profile */}
                <Panel title="OPERATOR PROFILE ASSESSMENT" color={C.cyan}>
                  <KV k="Probable Region"    v={intel.probable_region}   hi/>
                  <KV k="Threat Category"    v={intel.threat_note}       hi/>
                  <KV k="Sophistication"     v={intel.operator_sophistication}/>
                  {beh?.language_profile?.likely_language&&(
                    <KV k="Language"         v={beh.language_profile.likely_language} hi/>
                  )}
                  {beh?.behavioral_summary?.length>0&&(
                    <div style={{marginTop:8}}>
                      {beh.behavioral_summary.map((s,i)=>(
                        <div key={i} style={{color:C.text,fontSize:10,padding:"3px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>{s}</div>
                      ))}
                    </div>
                  )}
                </Panel>
                ...
                {/* Evidence breakdown */}
                <Panel title="EVIDENCE BREAKDOWN" color={C.green} status={`${intel.evidence_found_count}/${intel.evidence_total} found`}>
                  {intel.evidence_breakdown?.map((e,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"3px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                      <span style={{color:e.found?C.green:C.red,fontSize:11,minWidth:12}}>{e.found?"✓":"✗"}</span>
                      <span style={{color:e.found?"#fff":"#4a7a9b",fontSize:10,flex:1}}>{e.name}</span>
                      <span style={{color:"#4a7a9b",fontSize:9,minWidth:30,textAlign:"right"}}>w:{e.weight}</span>
                    </div>
                  ))}
                </Panel>
              </div>
              ...
              {/* Priority actions */}
              <Panel title="★ PRIORITY ACTIONS — Ranked by Attribution Value" color={C.red}>
                {intel.priority_actions?.map((a,i)=>(
                  <div key={i} style={{marginBottom:8,padding:"9px 12px",border:`1px solid ${RISK_C[a.priority]||C.red}25`,background:`${RISK_C[a.priority]||C.red}04`,display:"grid",gridTemplateColumns:"100px 1fr",gap:10}}>
                    <Badge risk={a.priority}/>
                    <div>
                      <div style={{color:"#fff",fontSize:11,fontWeight:"bold",marginBottom:2}}>{a.action}</div>
                      <div style={{color:"#4a7a9b",fontSize:10}}>{a.detail}</div>
                    </div>
                  </div>
                ))}
              </Panel>
        ...
        {tab==="opsec"&&(()=>{
          const opsec = report?.opsec_analysis;
          if (!opsec) return <div style={{padding:"40px 0",textAlign:"center",color:"#4a7a9b"}}>Run a scan to see OPSEC analysis.</div>;
          const opsecColor = opsec.opsec_score>=70?C.green:opsec.opsec_score>=40?C.orange:C.red;
          return (
            <div>
              {/* Score header */}
              <div style={{padding:"20px",border:`1px solid ${opsecColor}40`,background:`${opsecColor}06`,marginBottom:14,textAlign:"center"}}>
                <div style={{color:"#4a7a9b",fontSize:10,letterSpacing:2,marginBottom:8}}>OPERATOR OPSEC SOPHISTICATION</div>
                <div style={{color:opsecColor,fontSize:48,fontWeight:"bold",lineHeight:1}}>{opsec.opsec_score}</div>
                <div style={{color:"#4a7a9b",fontSize:11,marginTop:4}}>out of 100 — {opsec.opsec_rating}</div>
                <div style={{height:6,background:"rgba(255,255,255,0.06)",borderRadius:3,margin:"12px 0",overflow:"hidden"}}>
                  <div style={{height:"100%",width:`${opsec.opsec_score}%`,background:opsecColor,borderRadius:3}}/>
                </div>
                <div style={{display:"flex",justifyContent:"center",gap:20}}>
                  {Object.entries(opsec.by_severity||{}).map(([sev,cnt])=>(
                    <div key={sev} style={{textAlign:"center"}}>
                      <div style={{color:RISK_C[sev]||C.text,fontSize:20,fontWeight:"bold"}}>{cnt}</div>
                      <div style={{color:"#4a7a9b",fontSize:9}}>{sev}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Critical actions */}
              {opsec.critical_actions?.length>0&&(
                <Panel title="⚠ IMMEDIATE ACTIONS REQUIRED" color={C.red}>
                  {opsec.critical_actions.map((a,i)=>(
                    <div key={i} style={{padding:"7px 10px",borderLeft:`3px solid ${C.red}`,background:`${C.red}05`,marginBottom:5,fontSize:11,color:"#fff"}}>{a}</div>
                  ))}
                </Panel>
              )}
              ...
              {["CRITICAL","HIGH","MEDIUM","LOW"].map(sev=>{
                const filtered = opsec.failures?.filter(f=>f.severity===sev)||[];
                if (!filtered.length) return null;
                return (
                  <Panel key={sev} title={`${sev} FAILURES — ${filtered.length}`} color={RISK_C[sev]||C.text}>
                    {filtered.map((f,i)=>(
                      <div key={i} style={{marginBottom:10,padding:"10px 12px",border:`1px solid ${RISK_C[f.severity]||C.text}22`,background:`${RISK_C[f.severity]||C.text}04`}}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                          <span style={{color:RISK_C[f.severity]||C.text,fontSize:11,fontWeight:"bold"}}>[{f.category}] {f.title}</span>
                          <span style={{color:"#4a7a9b",fontSize:9}}>-{f.score_hit} pts</span>
                        </div>
                        <div style={{color:C.text,fontSize:10,lineHeight:1.6,marginBottom:4}}>{f.detail}</div>
                        {f.evidence&&<div style={{color:"#fff",fontSize:10,fontFamily:"monospace",padding:"3px 6px",background:"rgba(0,0,0,0.4)",marginBottom:4,wordBreak:"break-all"}}>{f.evidence}</div>}
                        <div style={{color:C.cyan,fontSize:10,padding:"4px 8px",border:`1px solid ${C.cyan}15`,background:`${C.cyan}05`}}>→ {f.action}</div>
                      </div>
                    ))}
                  </Panel>
                );
        ...
        {tab==="behavioral"&&(()=>{
          const beh = report?.behavioral;
          if (!beh) return <div style={{padding:"40px 0",textAlign:"center",color:"#4a7a9b"}}>Run a scan to see behavioral analysis.</div>;
          return (
            <div>
              {/* Summary row */}
              {beh.behavioral_summary?.length>0&&(
                <Panel title="◷ BEHAVIORAL SIGNALS SUMMARY" color={C.orange}>
                  {beh.behavioral_summary.map((s,i)=>(
                    <div key={i} style={{color:"#fff",fontSize:11,padding:"5px 0",borderBottom:"1px solid rgba(255,107,53,0.1)"}}>{s}</div>
                  ))}
                </Panel>
              )}
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                {/* Language profile */}
                <Panel title="◷ LANGUAGE / ORIGIN PROFILE" color={C.orange}>
                  {Object.entries(beh.language_profile||{}).map(([k,v])=>(
                    <KV key={k} k={k.replace(/_/g," ")} v={String(v)} hi={k==="likely_language"}/>
                  ))}
                </Panel>

                {/* Timezone signals */}
                <Panel title="◷ TIMEZONE SIGNALS" color={C.yellow}>
                  {beh.timezone_signals?.length>0 ? beh.timezone_signals.map((t,i)=>(
                    <div key={i} style={{padding:"5px 0",borderBottom:"1px solid rgba(255,204,0,0.1)"}}>
                      <KV k={t.signal} v={t.value} hi/>
                    </div>
                  )) : <div style={{color:"#4a7a9b",fontSize:10}}>No timezone signals detected</div>}
                  {beh.activity_window?.observed_hours_utc?.length>0&&(
                    <div style={{marginTop:8}}>
                      <div style={{color:C.yellow,fontSize:9,letterSpacing:1,marginBottom:4}}>ACTIVITY HOURS (UTC)</div>
                      <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
                        {Array.from({length:24},(_,h)=>(
                          <div key={h} style={{width:18,height:18,background:beh.activity_window.observed_hours_utc.includes(h)?C.yellow+"60":"rgba(255,255,255,0.05)",border:`1px solid ${beh.activity_window.peak_hour_utc===h?C.yellow:"transparent"}`,fontSize:7,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff"}}>{h}</div>
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {beh.timeline?.length>0&&(
                <Panel title="◷ TEMPORAL EVIDENCE TIMELINE" color={C.cyan} status={`${beh.timeline.length} timestamp(s)`}>
                  {beh.timeline.map((t,i)=>(
                    <div key={i} style={{padding:"6px 0",borderBottom:"1px solid rgba(0,200,255,0.08)",display:"grid",gridTemplateColumns:"180px 240px 1fr",gap:12}}>
                      <span style={{color:"#4a7a9b",fontSize:10}}>{t.source}</span>
                      <span style={{color:"#fff",fontSize:11,fontFamily:"monospace"}}>{t.value}</span>
                      <span style={{color:C.text,fontSize:10}}>{t.note}</span>
                    </div>
                  ))}
                </Panel>
              )}
          ...
          <div>
            ...
            {report&&!loading&&(
              <div>
                ...
                {report?.active_ip_discovery && (() => {
                  const disc = report.active_ip_discovery;
                  const discIPs = (disc.discovered_ips||[]).filter(g=>g.success);
                  const domains = disc.all_candidate_domains||[];
                  const shodan  = disc.shodan_queries||[];
                  if (discIPs.length === 0 && domains.length === 0 && shodan.length === 0) return null;
                  return (
                    <div style={{padding:"12px 16px",border:`1px solid ${C.cyan}30`,background:`${C.cyan}05`,marginBottom:14}}>
                      <div style={{color:C.cyan,fontSize:11,fontWeight:"bold",marginBottom:8}}>
                        ◎ ACTIVE IP DISCOVERY — {discIPs.length} IP{discIPs.length!==1?"s":""} via DNS · {domains.length} domains · {shodan.length} Shodan queries
                      </div>
                      {discIPs.length > 0 && (
                        <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:6}}>
                          {discIPs.map((g,i)=>(
                            <div key={i} style={{padding:"6px 10px",border:`1px solid ${g.is_proxy?C.orange:C.green}30`,background:`${g.is_proxy?C.orange:C.green}05`}}>
                              <div style={{color:"#fff",fontFamily:"monospace",fontSize:11}}>{g.ip}</div>
                              <div style={{color:C.text,fontSize:10}}>{g.city}, {g.country}</div>
                              <div style={{color:"#4a7a9b",fontSize:9}}>{g.source_domain}</div>
                            </div>
                          ))}
                        </div>
                      )}
                      {discIPs.length === 0 && (
                        <div style={{color:C.text,fontSize:10}}>No IPs from DNS — {shodan.length} Shodan queries ready in <span style={{color:C.cyan}}>◎ ACTIVE IP DISCOVERY</span> tab</div>
                      )}
                      <button onClick={()=>setTab("discover")} style={{marginTop:6,padding:"4px 12px",background:"transparent",border:`1px solid ${C.cyan}30`,color:C.cyan,cursor:"pointer",fontSize:10}}>
                        → Open Active IP Discovery Tab
                      </button>
                    </div>
                  );
                })()}
        ...
        {tab==="discover"&&(
          <div>
            {report?.active_ip_discovery ? (() => {
              const disc   = report.active_ip_discovery;
              const discIPs   = disc.discovered_ips || [];
              const domains   = disc.all_candidate_domains || [];
              const shodan    = disc.shodan_queries || [];
              const censys    = disc.censys_queries || [];
              const revIPs    = disc.reverse_ip || [];
              const vtLinks   = disc.virustotal_urls || [];
              const urlscans  = disc.urlscan_matches || [];
              const idbData   = disc.internetdb_data || [];
              const resols    = disc.domain_resolutions || [];
              const successIPs= discIPs.filter(g=>g.success||g.country);
              ...
              return (
                <div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:10,marginBottom:14}}>
                    {[
                      ["DOMAINS FOUND",   domains.length,                                         C.cyan],
                      ["IPs RESOLVED",    successIPs.length,                                      C.green],
                      ["URLSCAN HITS",    urlscans.filter(u=>u.ip).length,                        C.yellow],
                      ["SHODAN QUERIES",  shodan.length,                                           C.orange],
                      ["FINGERPRINTS",    (fp.searches||[]).length,                               C.purple],
                      ["REVERSE IP",      revIPs.filter(r=>r.success).length,                    "#4a7a9b"],
                    ].map(([label,val,color])=>(
                      <div key={label} style={{padding:"10px 12px",border:`1px solid ${color}30`,background:`${color}06`}}>
                        <div style={{color:"#4a7a9b",fontSize:9,letterSpacing:1,marginBottom:4}}>{label}</div>
                        <div style={{color,fontSize:24,fontWeight:"bold",lineHeight:1}}>{val}</div>
                      </div>
                    ))}
                  </div>

                  {/* How it works box */}
                  <div style={{padding:"10px 14px",border:`1px solid ${C.cyan}20`,background:`${C.cyan}04`,marginBottom:14,fontSize:10,color:"#4a7a9b",lineHeight:1.8}}>
                    <span style={{color:C.cyan,fontWeight:"bold"}}>How IP discovery actually works on Tor:</span> A correctly
                    configured hidden service never transmits its IP — that is Tor's cryptographic guarantee.
                    UMBRA uses <span style={{color:"#fff"}}>correlation</span>: it finds every clearnet domain the operator
                    references, resolves them via DNS, searches <span style={{color:C.yellow}}>urlscan.io</span> for
                    pages with the same title/build-hash/analytics ID, enriches IPs with
                    <span style={{color:C.yellow}}> Shodan InternetDB</span>, and generates ready-to-run Shodan/Censys queries.
                    This is how AlphaBay, Silk Road, and most major markets were deanonymised — OPSEC mistakes, not Tor exploits.
                  </div>

                  {/* Method summary */}
                  <Panel title="◎ DISCOVERY RESULTS SUMMARY" color={C.cyan}>
                    {disc.method_summary?.map((s,i)=>(
                      <div key={i} style={{color:s.includes("0")||s.includes("No")?C.yellow:C.green,fontSize:11,padding:"3px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>{s}</div>
                    ))}
                  </Panel>

                  {/* Resolved IPs */}
                  {successIPs.length > 0 ? (
                    <Panel title={`◎ DISCOVERED IPs — ${successIPs.length} GEOLOCATED`} color={C.green}
                      status="DNS resolution + urlscan.io">
                      {successIPs.map((g,i)=>(
                        <div key={i} style={{marginBottom:12,padding:14,border:`1px solid ${C.green}30`,background:`${C.green}04`}}>
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
                            <div>
                              <div style={{color:"#fff",fontSize:14,fontFamily:"monospace",fontWeight:"bold",marginBottom:8}}>{g.ip}</div>
                              <KV k="Method"     v={g.discovery_method||g.source} hi/>
                              <KV k="Source"     v={g.source_domain||"—"}/>
                              <KV k="Country"    v={`${g.country||"?"} (${g.country_code||"?"})`} hi/>
                              <KV k="City"       v={g.city||"?"} hi/>
                              <KV k="Region"     v={g.region||"?"}/>
                              <KV k="Timezone"   v={g.timezone||"?"}/>
                            </div>
                            <div>
                              <KV k="ISP"        v={g.isp||g.org||"?"} hi/>
                              <KV k="ASN"        v={g.asn||"?"} mono/>
                              <KV k="Hostname"   v={g.hostname||"—"} hi={!!g.hostname} mono/>
                              {g.open_ports?.length>0 && <KV k="Open Ports" v={g.open_ports.join(", ")} mono/>}
                              {g.hostnames?.length>0  && <KV k="PTR Records" v={g.hostnames.join(", ")} mono hi/>}
                              {g.vulns?.length>0      && <div style={{marginTop:6,padding:5,background:`${C.red}10`,border:`1px solid ${C.red}25`,color:C.red,fontSize:9}}>{g.vulns.length} CVE(s): {g.vulns.slice(0,3).join(", ")}</div>}
                              <div style={{marginTop:8}}>
                                {g.is_proxy &&  <div style={{color:C.red,fontSize:10,padding:"3px 6px",background:`${C.red}10`,border:`1px solid ${C.red}25`,marginBottom:3}}>⚠ VPN/PROXY</div>}
                                {g.is_hosting&& <div style={{color:C.orange,fontSize:10,padding:"3px 6px",background:`${C.orange}10`,border:`1px solid ${C.orange}25`,marginBottom:3}}>⚠ DATACENTER/VPS</div>}
                                {!g.is_proxy&&!g.is_hosting&&g.isp&&<div style={{color:C.green,fontSize:10,padding:"3px 6px",background:`${C.green}10`,border:`1px solid ${C.green}25`}}>✓ RESIDENTIAL ISP</div>}
                              </div>
                            </div>
                            <div>
                              <div style={{color:"#4a7a9b",fontSize:9,letterSpacing:1,marginBottom:6}}>LEGAL ACTION</div>
                              <div style={{color:C.text,fontSize:10,lineHeight:1.6,padding:6,background:"rgba(0,200,255,0.04)",border:`1px solid ${C.cyan}10`}}>
                                {g.legal_action || `Legal process → ${g.isp||"ISP"} for subscriber identity`}
                              </div>
                              <div style={{marginTop:8,display:"flex",gap:5,flexWrap:"wrap"}}>
                                {g.google_maps_url&&<a href={g.google_maps_url} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}30`,padding:"2px 6px"}}>Maps</a>}
                                <a href={`https://www.shodan.io/host/${g.ip}`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}30`,padding:"2px 6px"}}>Shodan</a>
                                <a href={`https://internetdb.shodan.io/${g.ip}`} target="_blank" rel="noreferrer" style={{color:C.yellow,fontSize:9,border:`1px solid ${C.yellow}30`,padding:"2px 6px"}}>InternetDB</a>
                                <a href={`https://viz.greynoise.io/ip/${g.ip}`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}30`,padding:"2px 6px"}}>GreyNoise</a>
                                <a href={`https://www.abuseipdb.com/check/${g.ip}`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}30`,padding:"2px 6px"}}>AbuseIPDB</a>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    <div style={{padding:"14px 16px",border:`1px solid ${C.yellow}30`,background:`${C.yellow}04`,marginBottom:14}}>
                      <div style={{color:C.yellow,fontSize:12,fontWeight:"bold",marginBottom:6}}>No IPs resolved via DNS — site has no clearnet domains or all are behind CDN</div>
                      <div style={{color:C.text,fontSize:11,lineHeight:1.7}}>
                        Use the <strong style={{color:C.yellow}}>Shodan Queries</strong> below.
                        Paste them at <a href="https://shodan.io" target="_blank" rel="noreferrer" style={{color:C.cyan}}>shodan.io</a> while logged in.
                        The <strong style={{color:C.yellow}}>favicon hash</strong> and <strong style={{color:C.yellow}}>page title</strong> queries are most reliable.
                        Also check urlscan.io results below — it may have scanned a clearnet mirror.
                      </div>
                    </div>
                  )}

                  {/* urlscan.io matches */}
                  {urlscans.filter(u=>u.ip).length > 0 && (
                    <Panel title={`◎ urlscan.io MATCHES — ${urlscans.filter(u=>u.ip).length} CLEARNET PAGE(S) FOUND`}
                      color={C.yellow} status="same title/fingerprint found on clearnet">
                      {urlscans.filter(u=>u.ip).map((m,i)=>(
                        <div key={i} style={{marginBottom:10,padding:12,border:`1px solid ${C.yellow}30`,background:`${C.yellow}05`}}>
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                            <div>
                              <div style={{color:"#fff",fontSize:12,fontFamily:"monospace",fontWeight:"bold",marginBottom:6}}>{m.ip}</div>
                              <KV k="Domain"  v={m.domain} hi mono/>
                              <KV k="Country" v={m.country} hi/>
                              <KV k="ASN"     v={m.asn} mono/>
                              <KV k="Server"  v={m.server||"—"}/>
                              <KV k="Scanned" v={m.scan_time?.slice(0,10)||"—"}/>
                            </div>
                            <div>
                              <div style={{color:"#4a7a9b",fontSize:9,marginBottom:6}}>This clearnet page has the same fingerprint as the onion site</div>
                              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                                {m.scan_url&&<a href={m.scan_url} target="_blank" rel="noreferrer" style={{color:C.yellow,fontSize:10,border:`1px solid ${C.yellow}30`,padding:"3px 8px"}}>→ urlscan Result</a>}
                                {m.screenshot&&<a href={m.screenshot} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:10,border:`1px solid ${C.cyan}30`,padding:"3px 8px"}}>→ Screenshot</a>}
                                <a href={`https://www.shodan.io/host/${m.ip}`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:10,border:`1px solid ${C.cyan}30`,padding:"3px 8px"}}>→ Shodan</a>
                              </div>
                            </div>
                          </div>
                        </div>
                  ...
                  {(fp.searches||[]).length > 0 && (
                    <Panel title={`◎ PAGE FINGERPRINTS — ${fp.searches.length} unique identifiers`}
                      color={C.purple} status="use these in Shodan / urlscan.io">
                      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:8}}>
                        {fp.searches.map((s,i)=>(
                          <div key={i} style={{padding:"8px 10px",border:`1px solid ${RISK_C[s.confidence]||C.purple}25`,background:`${RISK_C[s.confidence]||C.purple}04`}}>
                            <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                              <span style={{color:C.purple,fontSize:9,letterSpacing:1}}>{s.type}</span>
                              <Badge risk={s.confidence}/>
                            </div>
                            <div style={{color:"#fff",fontSize:11,fontFamily:"monospace",wordBreak:"break-all",marginBottom:4}}>{s.value}</div>
                            {s.shodan_q && (
                              <div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:4}}>
                                {s.shodan_url && <a href={s.shodan_url} target="_blank" rel="noreferrer" style={{color:C.yellow,fontSize:9,border:`1px solid ${C.yellow}30`,padding:"1px 6px"}}>→ Shodan</a>}
                                {s.urlscan_q && <a href={`https://urlscan.io/search/#${encodeURIComponent(s.urlscan_q)}`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}30`,padding:"1px 6px"}}>→ urlscan</a>}
                              </div>
                            )}
                            {s.note && <div style={{color:"#4a7a9b",fontSize:9,marginTop:3,lineHeight:1.4}}>{s.note}</div>}
                          </div>
                        ))}
                      </div>
                    </Panel>
                  ...
                  {/* Shodan queries */}
                  {shodan.length > 0 && (
                    <Panel title="◎ SHODAN QUERIES — Paste at shodan.io" color={C.yellow}>
                      {shodan.map((q,i)=>(
                        <div key={i} style={{marginBottom:10,padding:10,border:`1px solid ${RISK_C[q.confidence]||C.yellow}25`,background:`${RISK_C[q.confidence]||C.yellow}04`}}>
                          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                            <span style={{color:C.yellow,fontSize:10,fontWeight:"bold"}}>{q.method}</span>
                            <Badge risk={q.confidence}/>
                          </div>
                          <div style={{fontFamily:"monospace",fontSize:12,color:"#fff",padding:"5px 8px",background:"rgba(0,0,0,0.7)",userSelect:"all",marginBottom:5}}>{q.query}</div>
                          <a href={q.url} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:10,border:`1px solid ${C.cyan}30`,padding:"2px 8px",display:"inline-block"}}>→ Open Shodan</a>
                        </div>
                      ))}
                    </Panel>
                  )}
                  ...
                  {/* Censys queries */}
                  {censys.length > 0 && (
                    <Panel title="◎ CENSYS QUERIES — search.censys.io" color={C.purple}>
                      {censys.map((q,i)=>(
                        <div key={i} style={{marginBottom:8,padding:10,border:`1px solid ${C.purple}20`}}>
                          <div style={{fontFamily:"monospace",fontSize:11,color:"#fff",padding:"4px 8px",background:"rgba(0,0,0,0.5)",userSelect:"all",marginBottom:5}}>{q.query}</div>
                          <div style={{color:"#4a7a9b",fontSize:10,marginBottom:4}}>{q.method}</div>
                          <a href={q.url} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:10}}>→ Open Censys</a>
                        </div>
                      ))}
                    </Panel>
                  )}
                  ...
                  {/* Domain resolutions */}
                  {domains.length > 0 && (
                    <Panel title={`◎ CLEARNET DOMAINS — ${domains.length} found (CDN filtered)`} color={C.cyan}>
                      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:8}}>
                        {domains.map((d,i)=>{
                          const res = resols.find(r=>r.domain===d);
                          return (
                            <div key={i} style={{padding:"7px 10px",border:`1px solid ${res?.ips?.length?C.green:C.border}`,background:res?.ips?.length?`${C.green}05`:"transparent"}}>
                              <div style={{color:"#fff",fontSize:11,fontFamily:"monospace",marginBottom:3}}>{d}</div>
                              {res?.ips?.length ? <div style={{color:C.green,fontSize:10}}>✓ {res.ips.join(", ")}</div>
                                              : <div style={{color:"#4a7a9b",fontSize:10}}>Behind CDN / DNS blocked</div>}
                              <div style={{display:"flex",gap:4,marginTop:5,flexWrap:"wrap"}}>
                                <a href={`https://securitytrails.com/domain/${d}/history/a`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}25`,padding:"1px 5px"}}>Historical DNS</a>
                                <a href={`https://www.virustotal.com/gui/domain/${d}/details`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}25`,padding:"1px 5px"}}>VirusTotal</a>
                                <a href={`https://crt.sh/?q=${d}`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}25`,padding:"1px 5px"}}>crt.sh</a>
                              </div>
                            </div>
                          );
                        })}
                  ...
                  {/* Reverse IP */}
                  {revIPs.filter(r=>r.success).length > 0 && (
                    <Panel title="◎ REVERSE IP — Co-hosted Domains" color={C.orange}>
                      {revIPs.filter(r=>r.success).map((r,i)=>(
                        <div key={i} style={{marginBottom:8,padding:10,border:`1px solid ${C.orange}20`}}>
                          <div style={{color:C.orange,fontSize:11,fontWeight:"bold",marginBottom:6}}>{r.ip} — {r.count} co-hosted domain(s)</div>
                          <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                            {r.domains?.slice(0,15).map((d,j)=>(
                              <span key={j} style={{color:C.text,fontSize:10,fontFamily:"monospace",padding:"1px 6px",border:`1px solid ${C.border}`}}>{d}</span>
                            ))}
                          </div>
                        </div>
                      ))}
            ...
            })() : <div style={{padding:"40px 0",textAlign:"center",color:"#4a7a9b"}}>Run a scan to see active IP discovery results.</div>}
          </div>
        )}
        ...
        {/* ══ ATTRIBUTION GRAPH ═════════════════════════════════════════════ */}
          <div>
            {report?.active_ip_discovery ? (() => {
              ...
              const revIPs  = disc.reverse_ip || [];
              const vtLinks = disc.virustotal_urls || [];
              const resolutions = disc.domain_resolutions || [];
              const successIPs = discIPs.filter(g=>g.success);
                ...
                <div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10,marginBottom:14}}>
                    {[
                      ["DOMAINS FOUND",    domains.length,      C.cyan],
                      ["IPs RESOLVED",     successIPs.length,   C.green],
                      ["RESIDENTIAL IPs",  successIPs.filter(g=>!g.is_proxy&&!g.is_hosting).length, C.green],
                      ["SHODAN QUERIES",   shodan.length,       C.yellow],
                      ["REVERSE IP HITS",  revIPs.filter(r=>r.success).length, C.purple],
                    ].map(([label,val,color])=>(
                      <div key={label} style={{padding:"12px 14px",border:`1px solid ${color}30`,background:`${color}06`}}>
                        <div style={{color:"#4a7a9b",fontSize:9,letterSpacing:1,marginBottom:4}}>{label}</div>
                        <div style={{color,fontSize:26,fontWeight:"bold",lineHeight:1}}>{val}</div>
                  ...
                  {/* Method summary */}
                  <Panel title="◎ DISCOVERY METHOD SUMMARY" color={C.cyan}>
                    {disc.method_summary?.map((s,i)=>(
                      <div key={i} style={{color:C.text,fontSize:11,padding:"3px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>{s}</div>
                    ))}
                    <div style={{marginTop:10,padding:8,background:"rgba(0,200,255,0.05)",border:`1px solid ${C.cyan}15`,fontSize:10,color:"#4a7a9b",lineHeight:1.7}}>
                      <strong style={{color:C.cyan}}>How this works:</strong> A properly configured Tor hidden service never leaks its IP in headers or page source — that is Tor's design.
                      UMBRA uses correlation: it extracts every clearnet domain referenced in the page (href, src, CSP headers, JS fetch calls, HTML comments),
                      resolves them to real IPs via DNS + HackerTarget, then geolocates each one.
                      Shodan/Censys queries let you find the same server on the public internet using favicon hash, page title, and server fingerprints.
                    </div>
                  </Panel>

                  {/* Resolved IPs — main result */}
                  {successIPs.length > 0 ? (
                    <Panel title={`◎ RESOLVED IPs — ${successIPs.length} GEOLOCATED`} color={C.green}
                      status="from clearnet domain DNS resolution">
                      {successIPs.map((g,i)=>(
                        <IPCard key={i} g={g} expanded={true}/>
                      ))}
                    </Panel>
                  ) : (
                    <div style={{padding:"16px",border:`1px solid ${C.yellow}30`,background:`${C.yellow}04`,marginBottom:14}}>
                      <div style={{color:C.yellow,fontSize:12,fontWeight:"bold",marginBottom:8}}>No IPs resolved via DNS — this is normal for well-configured sites</div>
                      <div style={{color:C.text,fontSize:11,lineHeight:1.8}}>
                        The site may have no clearnet domains, or all its domains are behind Cloudflare/CDN.
                        Use the <strong style={{color:C.yellow}}>Shodan Queries</strong> below — paste them directly into shodan.io to find the real server.
                        The favicon hash query is the most powerful: it finds every server sharing the same icon.
                      </div>
                    </div>
                  )}
                  ...
                  {/* Shodan queries */}
                  {shodan.length > 0 && (
                    <Panel title="◎ SHODAN CORRELATION QUERIES — Copy & Paste to shodan.io" color={C.yellow}>
                      {shodan.map((q,i)=>(
                        <div key={i} style={{marginBottom:12,padding:12,border:`1px solid ${RISK_C[q.confidence]||C.yellow}30`,background:`${RISK_C[q.confidence]||C.yellow}05`}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                            <span style={{color:C.yellow,fontSize:11,fontWeight:"bold"}}>{q.method}</span>
                            <Badge risk={q.confidence}/>
                          </div>
                          <div style={{fontFamily:"Courier New,monospace",fontSize:12,color:"#fff",padding:"6px 10px",background:"rgba(0,0,0,0.6)",border:`1px solid ${C.yellow}20`,marginBottom:6,userSelect:"all"}}>
                            {q.query}
                          </div>
                          <a href={q.url} target="_blank" rel="noreferrer"
                            style={{color:C.cyan,fontSize:10,border:`1px solid ${C.cyan}30`,padding:"3px 10px",display:"inline-block"}}>
                            → Open in Shodan
                          </a>
                        </div>
                      ))}
                  ...
                  {/* Censys queries */}
                  {censys.length > 0 && (
                    <Panel title="◎ CENSYS QUERIES — censys.io" color={C.purple}>
                      {censys.map((q,i)=>(
                        <div key={i} style={{marginBottom:10,padding:10,border:`1px solid ${C.purple}25`}}>
                          <div style={{fontFamily:"Courier New,monospace",fontSize:11,color:"#fff",padding:"5px 8px",background:"rgba(0,0,0,0.5)",marginBottom:6,userSelect:"all"}}>{q.query}</div>
                          <div style={{color:"#4a7a9b",fontSize:10,marginBottom:4}}>{q.method}</div>
                          <a href={q.url} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:10}}>→ Open in Censys</a>
                        </div>
                      ))}
                    </Panel>
                  )}

                  {/* Clearnet domains found */}
                  {domains.length > 0 && (
                    <Panel title={`◎ CLEARNET DOMAINS EXTRACTED — ${domains.length} operator domain(s)`} color={C.cyan}
                      status="CDN/shared hosting filtered out">
                      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:8}}>
                        {domains.map((d,i)=>{
                          const res = resolutions.find(r=>r.domain===d);
                          return (
                            <div key={i} style={{padding:"8px 10px",border:`1px solid ${res?.ips?.length?C.green:C.border}`,background:res?.ips?.length?`${C.green}05`:"transparent"}}>
                              <div style={{color:"#fff",fontSize:11,fontFamily:"monospace",marginBottom:4}}>{d}</div>
                              {res?.ips?.length ? (
                                <div style={{color:C.green,fontSize:10}}>✓ Resolved: {res.ips.join(", ")}</div>
                              ) : (
                                <div style={{color:"#4a7a9b",fontSize:10}}>DNS pending / behind CDN</div>
                              )}
                              <div style={{display:"flex",gap:5,marginTop:5,flexWrap:"wrap"}}>
                                <a href={`https://www.virustotal.com/gui/domain/${d}/details`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}30`,padding:"1px 6px"}}>VT</a>
                                <a href={`https://securitytrails.com/domain/${d}/history/a`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}30`,padding:"1px 6px"}}>SecurityTrails</a>
                                <a href={`https://crt.sh/?q=${d}`} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:9,border:`1px solid ${C.cyan}30`,padding:"1px 6px"}}>crt.sh</a>
                                <a href={`https://api.hackertarget.com/reverseiplookup/?q=${d}`} target="_blank" rel="noreferrer" style={{color:C.yellow,fontSize:9,border:`1px solid ${C.yellow}30`,padding:"1px 6px"}}>Reverse IP</a>
                              </div>
                            </div>
                          );
                        })}
                  ...
                  {revIPs.filter(r=>r.success).length > 0 && (
                    <Panel title="◎ REVERSE IP — Co-hosted Domains" color={C.orange}
                      status="other sites on same server">
                      {revIPs.filter(r=>r.success).map((r,i)=>(
                        <div key={i} style={{marginBottom:10,padding:10,border:`1px solid ${C.orange}25`}}>
                          <div style={{color:C.orange,fontSize:11,fontWeight:"bold",marginBottom:6}}>{r.ip} — {r.count} domains co-hosted</div>
                          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                            {r.domains?.slice(0,20).map((d,j)=>(
                              <span key={j} style={{color:C.text,fontSize:10,fontFamily:"monospace",padding:"2px 6px",border:`1px solid ${C.border}`}}>{d}</span>
                            ))}
                          </div>
                          {r.count > 20 && <div style={{color:"#4a7a9b",fontSize:10,marginTop:4}}>+ {r.count-20} more</div>}
                        </div>
                      ))}
                    </Panel>
                  )}
                  ...
                  {vtLinks.length > 0 && (
                    <Panel title="◎ HISTORICAL DNS — Find IP Before CDN Was Added" color={C.purple}>
                      <div style={{color:"#4a7a9b",fontSize:10,marginBottom:10,lineHeight:1.6}}>
                        Operators often ran the server with a direct IP before adding Cloudflare.
                        SecurityTrails and VirusTotal show historical A records — the original IP may still be there.
                      </div>
                      {vtLinks.map((v,i)=>(
                        <div key={i} style={{marginBottom:10,padding:8,border:`1px solid ${C.purple}20`}}>
                          <div style={{color:"#fff",fontSize:11,fontFamily:"monospace",marginBottom:5}}>{v.domain}</div>
                          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                            <a href={v.virustotal} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:10,border:`1px solid ${C.cyan}30`,padding:"3px 8px"}}>→ VirusTotal</a>
                            <a href={v.securitytrails} target="_blank" rel="noreferrer" style={{color:C.cyan,fontSize:10,border:`1px solid ${C.cyan}30`,padding:"3px 8px"}}>→ SecurityTrails Historical A</a>
                          </div>
                        </div>
                      ))}
                    </Panel>
